...
VALIDATE $? "Unziping the catalogue files"

npm install &>>$LOG_FILE
VALIDATE $? "Installing the node package manager"

cp $SCRIPT_DIR/catalogue.service /etc/systemd/system/catalogue.service
VALIDATE $? "copying files from mongo.repo to catalogue.services"
...
STATUS=$(mongosh --host mongodb.kranthicart.fun --eval 'db.getMongo().getDBNames().indexOf("catalogue")')
if [ $STATUS -lt 0 ]
then
    mongosh --host mongodb.kranthicart.fun </app/db/master-data.js &>>$LOG_FILE
